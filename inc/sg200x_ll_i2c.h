/**
 * @file sg200x_ll_i2c.h
 * @brief 基于 SVD 的 DesignWare APB I2C 低层接口。
 *        SVD-derived DesignWare APB I2C low-level interfaces.
 * @ingroup SGLL_I2C
 *
 * @note 寄存器布局与字段由 sg2002.h 定义；调用者管理主机事务、FIFO 空间、时钟、引脚和 DMA。
 *       sg2002.h defines register layouts and fields; callers manage master transactions, FIFO space, clocks,
 * pins, and DMA.
 * @see sg2002-licheerv-nano-b.svd；I2C 寄存器描述。I2C register descriptions.
 * @see SG2002 技术参考手册 v1.02，表 21.1。SG2002 TRM v1.02, Table 21.1.
 */

/**
 * @defgroup SGLL_I2C I2C 主机 / I2C master
 * @ingroup SGLL
 */
#pragma once
...
/**
 * @brief 已停止 I2C 主机的初始化参数。
 *        Initialization parameters for a stopped I2C master.
 * @ingroup SGLL_I2C
 */
typedef struct
{
...
extern "C"
{
#endif
    /**
     * @brief 填充 I2C 主机默认初始化参数。
     *        Fill default I2C master initialization parameters.
     * @ingroup SGLL_I2C
     *
    ...
    /**
     * @brief 按 TRM 时序配置一个已停止的 I2C 主机。
     *        Configure a stopped I2C master using TRM timing profiles.
     * @ingroup SGLL_I2C
     *
     * @param i2c I2C 寄存器实例。
...
/**
 * @brief 按编号获取 I2C0 至 I2C4。
 *        Get I2C0 through I2C4 by index.
 * @ingroup SGLL_I2C
 *
...
/**
 * @brief 编码 I2C CON 配置字段。
 *        Encode I2C CON configuration fields.
 * @ingroup SGLL_I2C
 *
 * @param master 是否使能主机模式。
 *        Whether to enable master mode.
 * @param speed I2C 速率模式编码。
 *        I2C speed-mode encoding.
 * @param master_10bit 是否使用主机 10 位地址模式。
 *        Whether to use master ten-bit addressing.
 * @param restart 是否设置重复起始功能或命令位。
 *        Whether to set restart enable or the restart command bit.
 * @param slave_disable 是否禁止从机功能。
 *        Whether to disable slave functionality.
 * @return 可写入 CON 的配置值。
 *         Configuration value to write to CON.
 */
...
/**
 * @brief 写入 I2C CON 配置寄存器。
 *        Write the I2C CON configuration register.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 ...
 * @pre 修改 CON 前必须禁止控制器。
 *      Disable the controller before modifying CON.
 */
static inline void sgll_i2c_con_set(I2C_Type *i2c, uint32_t value) { i2c->CON = value; }

/**
 * @brief 设置 I2C 目标地址及 10 位地址选择位。
 *        Set the I2C target address and ten-bit address selector.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 * @param address 目标从机地址，低 10 位写入 TAR。
 *        Target slave address; the low ten bits are written to TAR.
 * @param ten_bit 是否设置 TAR 的 10 位地址选择位。
 *        Whether to set the TAR ten-bit address selector.
...
/**
 * @brief 构造一个 I2C DATA_CMD 命令字。
 *        Build an I2C DATA_CMD command word.
 * @ingroup SGLL_I2C
 *
 * @param data 命令中的低 8 位数据；读命令时数据字段不用于发送。
 *        Low eight data bits; the data field is not transmitted for a read command.
 * @param read 为 true 时构造读请求。
 *        True to build a read request.
 * @param stop 是否在该命令结束后生成 STOP。
 *        Whether to generate STOP after this command.
 * @param restart 是否设置重复起始功能或命令位。
 *        Whether to set restart enable or the restart command bit.
...
/**
 * @brief 向 I2C 发送 FIFO 提交 DATA_CMD 命令。
 *        Submit a DATA_CMD command to the I2C transmit FIFO.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 * @param command 由 DATA_CMD 构造器生成的命令字。
 *        Command word produced by the DATA_CMD builder.
 * @pre 调用者确认发送 FIFO 有空间。
 *      The caller must ensure that the transmit FIFO has space.
 */
static inline void sgll_i2c_data_command_write(I2C_Type *i2c, uint32_t command) { i2c->DATA_CMD = command; }

/**
 * @brief 读取屏蔽前的 I2C 中断状态。
 *        Read unmasked I2C interrupt status.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 * @return RAW_INTR_STAT 的当前值。
 *         Current RAW_INTR_STAT value.
 */
static inline uint32_t sgll_i2c_raw_interrupt_status_get(const I2C_Type *i2c) { return i2c->RAW_INTR_STAT; }

/**
 * @brief 读取屏蔽后的 I2C 中断状态。
 *        Read masked I2C interrupt status.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 * @return INTR_STAT 的当前值。
 *         Current INTR_STAT value.
 */
static inline uint32_t sgll_i2c_interrupt_status_get(const I2C_Type *i2c) { return i2c->INTR_STAT; }

/**
 * @brief 通过读 CLR_INTR 清除 I2C 中断。
 *        Clear I2C interrupts by reading CLR_INTR.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
...
/**
 * @brief 写入 I2C 中断使能掩码。
 *        Write the I2C interrupt-enable mask.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 * @param mask 写入 INTR_MASK 的中断使能位掩码。
 *        Interrupt-enable mask written to INTR_MASK.
 */
static inline void sgll_i2c_interrupt_mask_set(I2C_Type *i2c, uint32_t mask) { i2c->INTR_MASK = mask; }

/**
 * @brief 请求使能或禁止 I2C 控制器。
 *        Request enable or disable of the I2C controller.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
...
/**
 * @brief 读取 I2C 控制器的实际使能状态。
 *        Read the I2C controller's actual enable state.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 * @return ENABLE_STATUS 的使能位为 1 时返回 true。
 *         True when the enable bit in ENABLE_STATUS is set.
 */
...
/**
 * @brief 读取 I2C 发送 FIFO 的条目数。
 *        Read the number of entries in the I2C transmit FIFO.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 * @return TXFLR 中的条目数。
 *         Entry count reported by TXFLR.
 */
static inline uint32_t sgll_i2c_tx_fifo_level_get(const I2C_Type *i2c) { return i2c->TXFLR; }

/**
 * @brief 读取 I2C 接收 FIFO 的条目数。
 *        Read the number of entries in the I2C receive FIFO.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
 * @return RXFLR 中的条目数。
 *         Entry count reported by RXFLR.
 */
static inline uint32_t sgll_i2c_rx_fifo_level_get(const I2C_Type *i2c) { return i2c->RXFLR; }

/**
 * @brief 配置 I2C 的接收和发送 DMA 请求。
 *        Configure I2C receive and transmit DMA requests.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
...
/**
 * @brief 关闭 I2C 的接收和发送 DMA 请求。
 *        Disable I2C receive and transmit DMA requests.
 * @ingroup SGLL_I2C
 *
 * @param i2c I2C 寄存器实例。
 *        I2C register instance.
...
extern "C"
{
     ...
     * @ingroup SGLL_I2C
     * @param peripheral_clock_hz 实际 I2C 输入时钟，单位 Hz。
     *        Actual I2C input clock in Hz.
     * @param [out] timing 接收时序计算结果。
     *        Receives calculated timing.
    ...
    /**
     * @brief 使用显式时序配置一个已停止的 I2C 主机。
     *        Configure a stopped I2C master with explicit timing.
     * @ingroup SGLL_I2C
     * @param i2c I2C0 至 I2C4 寄存器实例。
     *        I2C0 through I2C4 register instance.
     * @param config 主机、地址和 FIFO 参数。
     *        Master, address, and FIFO parameters.
    ...
    /**
     * @brief 请求 I2C 使能状态并有界等待确认。
     *        Request an I2C enable state and wait for bounded confirmation.
     * @ingroup SGLL_I2C
     * @param i2c I2C0 至 I2C4 寄存器实例。
     *        I2C0 through I2C4 register instance.
...
/**
 * @brief 同时配置 CON 和 TAR 的目标寻址模式。
 *        Configure target addressing in both CON and TAR.
 * @ingroup SGLL_I2C
 * @param i2c I2C0 至 I2C4 寄存器实例。
 *        I2C0 through I2C4 register instance.
 * @param address 符合所选模式的目标地址。
 *        Target address fitting the selected mode.
 * @param ten_bit 是否使用十位地址。
 *        Whether to use ten-bit addressing.
...
/**
 * @brief 配置 I2C 收发 DMA 请求阈值。
 *        Configure I2C receive/transmit DMA request thresholds.
 * @ingroup SGLL_I2C
 * @param i2c I2C0 至 I2C4 寄存器实例。
 *        I2C0 through I2C4 register instance.
 * @param transmit 发送阈值，小于 I2C_FIFO_DEPTH。
 *        Transmit threshold, below I2C_FIFO_DEPTH.
 * @param receive 接收阈值，小于 I2C_FIFO_DEPTH。
 *        Receive threshold, below I2C_FIFO_DEPTH.
 * @pre 控制器已停止且阈值有效。
 *        The controller is stopped and thresholds are valid.
 */
...
/**
 * @brief 清除 I2C 发送中止状态。
 *        Clear I2C transmit-abort status.
 * @ingroup SGLL_I2C
 * @param i2c I2C0 至 I2C4 寄存器实例。
 *        I2C0 through I2C4 register instance.
 * @return 清除寄存器读回值。
 *        Clear-register readback value.
 * @note 读取即清除该状态。
 *        Reading clears the corresponding status.
 */
static inline uint32_t sgll_i2c_abort_clear(I2C_Type *i2c) { return i2c->CLR_TX_ABRT; }

/**
 * @brief 清除 I2C STOP 检测状态。
 *        Clear I2C STOP-detection status.
 * @ingroup SGLL_I2C
 * @param i2c I2C0 至 I2C4 寄存器实例。
 *        I2C0 through I2C4 register instance.
 * @return 清除寄存器读回值。
...
/**
 * @brief 获取供 DMA 使用的 DATA_CMD 地址。
 *        Get the DATA_CMD address for DMA.
 * @ingroup SGLL_I2C
 * @param i2c I2C0 至 I2C4 寄存器实例。
 *        I2C0 through I2C4 register instance.
 * @return DATA_CMD 寄存器地址，不执行读取。
 *        DATA_CMD register address; no read is performed.
 */
