/**
 * @file sg200x_ll_wdt.h
 * @brief 看门狗低层接口。
 *        Watchdog low-level interfaces.
 * @ingroup SGLL_WDT
 ...
 */
/**
 * @defgroup SGLL_WDT 看门狗 / Watchdog
 * @ingroup SGLL
 */
#pragma once
...
/**
 * @brief 看门狗指数计数配置。
 *        Watchdog exponential-counter configuration.
 */
typedef struct
{
    uint32_t
...
/**
 * @brief 按编号获取主域看门狗。
 *        Get a main-domain watchdog by index.
 * @param index 看门狗编号，0 至 2。
 *        Watchdog index from zero through two.
...
/**
 * @brief 检查看门狗是否已使能。
 *        Check whether the watchdog is enabled.
...
/**
 * @brief 计算指数模式的超时周期数。
 *        Calculate timeout cycles in exponential mode.
 * @param top 指数超时范围编码，0 至 WDT_TOP_MAX。
 *        Exponential timeout-range encoding, zero through WDT_TOP_MAX.
 * @return 输入时钟周期数。
 *        Input clock cycles.
 * @pre top 不超过 WDT_TOP_MAX。
 *        top does not exceed WDT_TOP_MAX.
 */
...
/**
 * @brief 设置相同的初始和重装超时范围。
 *        Set equal initial and reload timeout ranges.
 * @param wdt 主域 WDT0 至 WDT2 寄存器实例。
 *        Main-domain WDT0 through WDT2 register instance.
 * @param top 指数超时范围编码，0 至 WDT_TOP_MAX。
 *        Exponential timeout-range encoding, zero through WDT_TOP_MAX.
 * @pre top 不超过 WDT_TOP_MAX。
 *        top does not exceed WDT_TOP_MAX.
 * @note 正在运行时的新周期在下次喂狗后生效。
 *        A new running timeout takes effect after the next feed.
 */
...
/**
 * @brief 重载看门狗计数器并清除中断。
 *        Reload the watchdog counter and clear its interrupt.
 * @param wdt 主域 WDT0 至 WDT2 寄存器实例。
 *        Main-domain WDT0 through WDT2 register instance.
 * @note 写入硬件规定的重启密钥。
 *        Writes the hardware restart key.
 */
...
extern "C"
{
    ...
    /**
     * @brief 配置一个尚未使能的看门狗。
     *        Configure a watchdog that has not been enabled.
     * @param wdt 主域 WDT0 至 WDT2 寄存器实例。
     *        Main-domain WDT0 through WDT2 register instance.
    ...
    /**
     * @brief 重载并启动一个尚未使能的看门狗。
     *        Reload and start a watchdog that has not been enabled.
     * @param wdt 主域 WDT0 至 WDT2 寄存器实例。
     *        Main-domain WDT0 through WDT2 register instance.
     * @param top 指数超时范围编码，0 至 WDT_TOP_MAX。
     *        Exponential timeout-range encoding, zero through WDT_TOP_MAX.
     ...
     * @pre 已准备输入时钟和复位路由。
     *        The input clock and reset routing are prepared.
    ...
    /**
     * @brief 为一个看门狗选择 CPU 或系统复位路由。
     *        Select CPU or system reset routing for one watchdog.
     * @param index 主域看门狗编号，0 至 2。
     *        Main-domain watchdog index from zero through two.
