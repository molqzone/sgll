/**
 * @file sg200x_ll_rcc.h
 * @brief CLKGEN、G6 PLL 和 RSTGEN 的寄存器原语。
 *        Register primitives for CLKGEN, G6 PLL, and RSTGEN.
 * @ingroup SGLL_RCC
 *
 * @note 设备地址、字段位置和各分支宽度集中在 sg2002.h；本层提供寄存器操作。
 *       Device addresses, field positions, and per-branch widths are centralized in sg2002.h; this layer
 * supplies register operations.
 * @note 分频器位 0 释放复位，位 3 选择寄存器因子，位 9:8 选择父时钟，因子字段从位 16 开始。
 *       Divider bit zero releases reset, bit three selects the programmed factor, bits 9:8 select the parent,
 * and the factor field starts at bit sixteen.
 * @note TRM 通用标注为 [20:16]；AXI4/AXI6/I2C 使用 4 位，1M/SPI/PWM 使用 6
 * 位，以器件和已验证时钟提供者定义为准。 The TRM gives generic [20:16] notation; AXI4/AXI6/I2C use four bits
 * and 1M/SPI/PWM six, following the device and validated clock-provider definitions.
 * @note 旁路位置 1 选择 25 MHz 晶振，复位默认旁路；启动固件在 PLL 就绪后恢复正常路径。
 *       A bypass bit of one selects the 25 MHz crystal and is the reset default; boot firmware restores the
 * normal path after PLL setup.
 * @note PLL、核心选源和核心分频属于启动固件契约。通用 CLKGEN 写原语不检查所有权，上层必须限制可写字段。
 *       PLL, core source selection, and core dividers belong to the boot contract. Generic CLKGEN writes do
 * not check ownership; upper layers must restrict writable fields.
 * @note 用于启动或线程控制路径；读改写不原子。断言模块复位前必须停止其总线和 DMA 活动。
 *       Use in startup or thread control paths; read-modify-write operations are not atomic. Stop a module's
 * bus and DMA activity before asserting reset.
 * @see SG2002 技术参考手册 v1.02，表 7.1-7.5、8.51-8.59、8.79、8.88-8.90、8.93。SG2002 TRM register tables.
 */

/**
 * @defgroup SGLL_RCC 时钟与复位 / Clock and reset
 * @ingroup SGLL
 */
...
#endif
    /**
     * @brief 将 CLKGEN 偏移解析为具名寄存器地址。
     *        Resolve a CLKGEN offset to a named register address.
     * @ingroup SGLL_RCC
     *
...
/**
 * @brief 解析 CLK_EN0 至 CLK_EN4 的寄存器偏移。
 *        Resolve a CLK_EN0 through CLK_EN4 register offset.
 * @ingroup SGLL_RCC
 *
 * @param offset 相对于相应寄存器块基址的字节偏移。
 *        Byte offset from the relevant register-block base.
 ...
 * @param offset 相对于相应寄存器块基址的字节偏移。
 *        Byte offset from the relevant register-block base.
 * @return 具名寄存器地址；未知偏移返回空指针。
 *         Named register address, or null for an unknown offset.
...
/**
 * @brief 读取具名 CLKGEN 寄存器。
 *        Read a named CLKGEN register.
 * @ingroup SGLL_RCC
 *
 * @param offset 相对于相应寄存器块基址的字节偏移。
 *        Byte offset from the relevant register-block base.
 * @return 寄存器值；偏移未知时返回 0。
 *         Register value, or zero for an unknown offset.
 */
...
/**
 * @brief 写入具名 CLKGEN 寄存器。
 *        Write a named CLKGEN register.
 * @ingroup SGLL_RCC
 *
 * @param offset 相对于相应寄存器块基址的字节偏移。
 *        Byte offset from the relevant register-block base.
...
/**
 * @brief 读取 CLKGEN 寄存器中的位字段。
 *        Read a bit field from a CLKGEN register.
 * @ingroup SGLL_RCC
 *
 * @param offset 相对于相应寄存器块基址的字节偏移。
 *        Byte offset from the relevant register-block base.
 ...
 * @return 右对齐字段值；偏移未知时为 0。
 *         Right-aligned field value, or zero for an unknown offset.
...
/**
 * @brief 读改写 CLKGEN 单个位并检查读回值。
 *        Modify one CLKGEN bit and check readback.
 * @ingroup SGLL_RCC
 *
 * @param offset 相对于相应寄存器块基址的字节偏移。
 *        Byte offset from the relevant register-block base.
 * @param bit 寄存器位号，范围为 0 到 31。
 *        Register bit index from 0 through 31.
 * @param enabled 目标位的使能状态。
 *        Requested enable state of the bit.
 * @return 读回状态匹配时为 true；否则为 false。
 *         True when the readback state matches; false otherwise.
 * @note false 可能来自写入后的读回不匹配；此接口不执行回滚。
 *       false may indicate mismatched readback after a write; this interface does not roll back changes.
 */
...
/**
 * @brief 读取一个已定义的 G6 PLL 控制寄存器。
 *        Read one defined G6 PLL control register.
 * @ingroup SGLL_RCC
 *
 * @param offset 相对于相应寄存器块基址的字节偏移。
 *        Byte offset from the relevant register-block base.
 * @return MPLL、TPLL 或 FPLL 的值；偏移未知时为 0。
 *         MPLL, TPLL, or FPLL value, or zero for an unknown offset.
 */
...
/**
 * @brief 获取指定 SOFT_RSTN 寄存器。
 *        Get a selected SOFT_RSTN register.
 * @ingroup SGLL_RCC
 *
 * @param index SOFT_RSTN 数组编号，小于 RSTGEN_SOFT_RSTN_COUNT。
 *        SOFT_RSTN array index below RSTGEN_SOFT_RSTN_COUNT.
 * @return 寄存器地址；编号无效时返回空指针。
 *         Register address, or null for an invalid index.
 */
...
/**
 * @brief 使能指定外设时钟门控位。
 *        Enable a selected peripheral clock gate.
 * @ingroup SGLL_RCC
 *
 * @param gate_reg CLK_EN 门控寄存器的字节偏移。
 *        Byte offset of the CLK_EN gate register.
 * @param gate_bit 门控位号，范围为 0 到 31。
 *        Gate bit index from 0 through 31.
 */
...
/**
 * @brief 禁止指定外设时钟门控位。
 *        Disable a selected peripheral clock gate.
 * @ingroup SGLL_RCC
 *
 * @param gate_reg CLK_EN 门控寄存器的字节偏移。
 *        Byte offset of the CLK_EN gate register.
...
/**
 * @brief 读取指定时钟门控位。
 *        Read a selected clock-gate bit.
 * @ingroup SGLL_RCC
 *
 * @param gate_reg CLK_EN 门控寄存器的字节偏移。
 *        Byte offset of the CLK_EN gate register.
 * @param gate_bit 门控位号，范围为 0 到 31。
 *        Gate bit index from 0 through 31.
 * @return 门控已使能时为 true；偏移无效时为 false。
 *         True when enabled; false for an invalid offset.
 */
...
/**
 * @brief 使指定时钟分支旁路到晶振。
 *        Bypass a selected clock branch to the crystal source.
 * @ingroup SGLL_RCC
 *
 * @param bypass_reg CLK_BYP0 或 CLK_BYP1 的字节偏移。
 *        Byte offset of CLK_BYP0 or CLK_BYP1.
 * @param bypass_bit 旁路位号，范围为 0 到 31。
 *        Bypass bit index from 0 through 31.
 */
...
/**
 * @brief 恢复指定时钟分支的正常父时钟路径。
 *        Restore the normal parent path of a selected clock branch.
 * @ingroup SGLL_RCC
 *
 * @param bypass_reg CLK_BYP0 或 CLK_BYP1 的字节偏移。
 *        Byte offset of CLK_BYP0 or CLK_BYP1.
...
/**
 * @brief 检查时钟分支是否旁路到晶振。
 *        Check whether a clock branch bypasses to the crystal.
 * @ingroup SGLL_RCC
 *
 * @param bypass_reg CLK_BYP0 或 CLK_BYP1 的字节偏移。
 *        Byte offset of CLK_BYP0 or CLK_BYP1.
...
/**
 * @brief 读取 C906_0 的路径 0 选择位。
 *        Read the C906_0 path-zero selection bit.
 * @ingroup SGLL_RCC
 *
 * @return 路径 0 选择位置位时返回 true。
 *         True when the path-zero selection bit is set.
 */
...
/**
 * @brief 读取 C906_1 的路径 0 选择位。
 *        Read the C906_1 path-zero selection bit.
 * @ingroup SGLL_RCC
 *
 * @return 路径 0 选择位置位时返回 true。
 *         True when the path-zero selection bit is set.
...
/**
 * @brief 读取分频因子的原始寄存器字段。
 *        Read the raw register field of a divider factor.
 * @ingroup SGLL_RCC
 *
 * @param div_offset 器件头定义的分频器寄存器字节偏移。
 *        Divider register byte offset defined by the device header.
 * @param factor_width 该时钟分支的因子字段位宽。
 *        Factor-field width of this clock branch.
...
/**
 * @brief 检查分频器是否选择寄存器中的因子。
 *        Check whether a divider selects its register-programmed factor.
 * @ingroup SGLL_RCC
 *
 * @param div_offset 器件头定义的分频器寄存器字节偏移。
 *        Divider register byte offset defined by the device header.
 * @return 因子选择位置位时返回 true。
 *         True when the register-factor selector is set.
 */
...
/**
 * @brief 检查分频器复位是否已释放。
 *        Check whether divider reset is deasserted.
 * @ingroup SGLL_RCC
 *
 * @param div_offset 器件头定义的分频器寄存器字节偏移。
 *        Divider register byte offset defined by the device header.
 ...
 * @param factor_width 该时钟分支的因子字段位宽。
 *        Factor-field width of this clock branch.
 * @param factor 硬件允许的分频因子。
 *        Divider factor permitted by the hardware.
 */
...
/**
 * @brief 拉低分频器复位位。
 *        Assert divider reset by clearing its reset bit.
 * @ingroup SGLL_RCC
 *
 * @param div_offset 器件头定义的分频器寄存器字节偏移。
 *        Divider register byte offset defined by the device header.
...
/**
 * @brief 置位分频器复位释放位。
 *        Deassert divider reset by setting its reset bit.
 * @ingroup SGLL_RCC
 *
 * @param div_offset 器件头定义的分频器寄存器字节偏移。
 *        Divider register byte offset defined by the device header.
...
/**
 * @brief 读取分频叶节点的父时钟选择字段。
 *        Read a divider leaf's parent-clock selector.
 * @ingroup SGLL_RCC
 *
 * @param div_offset 器件头定义的分频器寄存器字节偏移。
 *        Divider register byte offset defined by the device header.
 * @return 原始时钟源编码；未知偏移时为 0。
 *         Raw source encoding, or zero for an unknown offset.
 */
...
/**
 * @brief 写入分频叶节点的父时钟选择字段。
 *        Write a divider leaf's parent-clock selector.
 * @ingroup SGLL_RCC
 *
 * @param div_offset 器件头定义的分频器寄存器字节偏移。
 *        Divider register byte offset defined by the device header.
...
extern "C"
{
    ...
    /**
     * @brief 查询复位目标的寄存器及位坐标。
     *        Look up a reset target's register and bit coordinates.
     * @ingroup SGLL_RCC
     *
     * @param target 器件复位目标枚举。
     *        Device reset-target enumeration.
...
/**
 * @brief 检查复位目标是否具有有效坐标。
 *        Check whether a reset target has valid coordinates.
 * @ingroup SGLL_RCC
 *
 * @param target 器件复位目标枚举。
 *        Device reset-target enumeration.
...
/**
 * @brief 拉低目标模块的软复位位。
 *        Assert a target module's active-low soft reset.
 * @ingroup SGLL_RCC
 *
 * @param target 器件复位目标枚举。
 *        Device reset-target enumeration.
...
/**
 * @brief 置位目标模块的软复位释放位。
 *        Deassert a target module's active-low soft reset.
 * @ingroup SGLL_RCC
 *
 * @param target 器件复位目标枚举。
 *        Device reset-target enumeration.
...
/**
 * @brief 检查目标模块的软复位是否释放。
 *        Check whether a target module's soft reset is released.
 * @ingroup SGLL_RCC
 *
 * @param target 器件复位目标枚举。
 *        Device reset-target enumeration.
...
/**
 * @brief 选择主域看门狗的共享计数时钟。
 *        Select the shared counter clock of main-domain watchdogs.
 * @ingroup SGLL_RCC
