/**
 * @file sg200x_ll_adc.h
 * @brief SARADC低层接口。
 *        SARADC low-level interfaces.
 * @ingroup SGLL_ADC
 ...
 */
/**
 * @defgroup SGLL_ADC SARADC / SARADC
 * @ingroup SGLL
 */
#pragma once
...
/**
 * @brief SARADC 时钟及参考电压配置。
 *        SARADC clock and reference-voltage configuration.
 */
typedef struct
{
...
/**
 * @brief 按电源域获取 SARADC 实例。
 *        Get a SARADC instance by power domain.
 * @param index 零为主域，一为 RTC 域。
 *        Zero for the main domain; one for the RTC domain.
...
/**
 * @brief 读取 SARADC 转换忙状态。
 *        Read SARADC conversion-busy status.
 * @param adc 主域或 RTC 域 SARADC 实例。
 *        Main-domain or RTC-domain SARADC instance.
 * @return 转换活动时返回 true。
 *        True while a conversion is active.
 */
...
/**
 * @brief 读取一个有效的 SARADC 结果。
 *        Read one valid SARADC result.
 * @param adc 主域或 RTC 域 SARADC 实例。
 *        Main-domain or RTC-domain SARADC instance.
 * @param channel 域内物理输入通道，范围 1 至 3。
 *        Physical input channel within the domain, from one through three.
 * @param [out] value 有效时写入 12 位采样值。
 *        Receives the twelve-bit sample when valid.
...
extern "C"
{
    ...
    /**
     * @brief 填充 SARADC 默认配置。
     *        Fill SARADC defaults.
     * @param config 接收默认配置，可为空。
     *        Destination for defaults; may be null.
     * @note 默认采用十六分频及内部参考电压。
     *        Defaults select divide-by-sixteen and the internal reference.
     */
    void sgll_adc_struct_init(sgll_adc_init_t *config);

    /**
     * @brief 初始化一个空闲 SARADC 实例。
     *        Initialize one idle SARADC instance.
     * @param adc 主域或 RTC 域 SARADC 实例。
     *        Main-domain or RTC-domain SARADC instance.
    ...
    /**
     * @brief 选择物理通道并触发一次转换。
     *        Select a physical channel and trigger one conversion.
     * @param adc 主域或 RTC 域 SARADC 实例。
     *        Main-domain or RTC-domain SARADC instance.
     * @param channel 域内物理输入通道，范围 1 至 3。
     *        Physical input channel within the domain, from one through three.
    ...
    /**
     * @brief 有界等待 SARADC 空闲。
     *        Wait for SARADC idle with a bounded attempt count.
     * @param adc 主域或 RTC 域 SARADC 实例。
     *        Main-domain or RTC-domain SARADC instance.
     * @param attempts 最多轮询次数，零表示不轮询。
     *        Maximum polling attempts; zero performs no polling.
