 ...
 * @ingroup SGLL_CSR
 *
 * @note CSR 操作基于 SDK core_rv64.h 和 csi_rv64_gcc.h；RISC-V 构建需要 SDK arch 包含目录。
 *       CSR operations use SDK core_rv64.h and csi_rv64_gcc.h; RISC-V builds require the SDK arch include
 * directory.
 ...
 * words and 64-byte range loops from SDK cache.c.
 ...
 */
static inline unsigned long sgll_csr_mstatus_read(void) { return (unsigned long)__get_MSTATUS(); }
 ...
 */
static inline void sgll_csr_mstatus_write(unsigned long value) { __set_MSTATUS((uint64_t)value); }
 ...
 *         Current mie value.
 */
static inline unsigned long sgll_csr_mie_read(void) { return (unsigned long)__get_MIE(); }
 ...
 *        Value to write or encode.
 */
static inline void sgll_csr_mie_write(unsigned long value) { __set_MIE((uint64_t)value); }
 ...
 */
static inline void sgll_csr_mie_set(unsigned long mask) { __set_MIE(__get_MIE() | (uint64_t)mask); }
 ...
 *       This read-modify-write operation is not atomic; disable interrupts or serialize it externally.
 */
static inline void sgll_csr_mie_clear(unsigned long mask) { __set_MIE(__get_MIE() & ~(uint64_t)mask); }
 ...
 */
static inline void sgll_csr_global_irq_enable(void) { __enable_irq(); }
 ...
 * @ingroup SGLL_CSR
 */
static inline void sgll_csr_global_irq_disable(void) { __disable_irq(); }
 ...
 * SDK.
 * @note RV32 路径通过高低字重读避免进位撕裂；C906L 使用 RV64 路径。
 *       The RV32 path rereads the high word to avoid rollover tearing; C906L uses the RV64 path.
 */
