/**
 * @file sg200x_ll_csr.c
 * @brief C906 数据缓存范围维护的独立实现。
 *        Compiled C906 data-cache range maintenance.
 *
 * @ingroup SGLL_CSR
 * @see SG2002 SDK arch/riscv64/src/cache.c；指令编码来源。Instruction-encoding source.
 */
...
/**
 * @def DCACHE_IPA_A0
 * @brief 使用 a0 地址的缓存失效指令编码。
 *        Cache-invalidate instruction encoding using the address in a0.
 */
#define DCACHE_IPA_A0 ".long 0x02a5000b"
/**
 * @def DCACHE_CPA_A0
 * @brief 使用 a0 地址的缓存写回指令编码。
 *        Cache-clean instruction encoding using the address in a0.
 */
#define DCACHE_CPA_A0 ".long 0x0295000b"
/**
 * @def DCACHE_CIPA_A0
 * @brief 使用 a0 地址的缓存写回并失效指令编码。
 *        Cache-clean-and-invalidate encoding using the address in a0.
 */
 ...
 *
 * @param OP 使用 a0 的指令字符串。
 *        Instruction string using a0.
 * @param addr 范围起始地址。
 *        Range start address.
