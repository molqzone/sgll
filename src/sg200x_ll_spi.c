...
void sgll_spi_struct_init(sgll_spi_init_t *config)
{
    ...
    config->mode = SGLL_SPI_MODE_0;
    config->transfer_mode = SGLL_SPI_TRANSFER_MODE_TX_RX;
    config->frame_format = SGLL_SPI_FRAME_FORMAT_MOTOROLA;
    config->data_frame_size = SGLL_SPI_DATA_FRAME_SIZE_8_BIT;
    config->control_frame_size = SGLL_SPI_CONTROL_FRAME_SIZE_1_BIT;
...
{
    if (config == NULL || spi_reset_target(spi) == RESET_NONE || (uint32_t)config->mode > SGLL_SPI_MODE_3 ||
        (uint32_t)config->transfer_mode > SGLL_SPI_TRANSFER_MODE_EEPROM_READ ||
        (uint32_t)config->frame_format > SGLL_SPI_FRAME_FORMAT_NS_MICROWIRE ||
        (uint32_t)config->data_frame_size < SGLL_SPI_DATA_FRAME_SIZE_4_BIT ||
        (uint32_t)config->data_frame_size > SGLL_SPI_DATA_FRAME_SIZE_16_BIT ||
        (uint32_t)config->control_frame_size > SGLL_SPI_CONTROL_FRAME_SIZE_16_BIT ||
        config->baud_divider < SPI_BAUDR_MIN || !sgll_spi_baud_divider_is_valid(config->baud_divider) ||
    ...
    if (sgll_spi_is_enabled(spi))
    {
        return false;
    }

    const sgll_spi_clock_polarity_t polarity =
        (config->mode & 2U) != 0U ? SGLL_SPI_CLOCK_POLARITY_HIGH : SGLL_SPI_CLOCK_POLARITY_LOW;
    const sgll_spi_clock_phase_t phase =
        (config->mode & 1U) != 0U ? SGLL_SPI_CLOCK_PHASE_SECOND_EDGE : SGLL_SPI_CLOCK_PHASE_FIRST_EDGE;
    sgll_spi_disable(spi);
    spi->IMR = 0U;
    spi->DMACR = 0U;
    spi->SER = 0U;
    spi->CTRLR0 = sgll_spi_ctrlr0_build(
    ...
    (void)sgll_spi_interrupt_clear(spi);
    sgll_csr_fence_io();
    return true;
}
...
bool sgll_spi_deinit(spi_t *spi)
{
    ...
    if (reset == RESET_NONE || sgll_spi_is_enabled(spi))
    {
        return false;
    }
    spi->IMR = 0U;
